...
def stage():
    ...
    stage.character_objects.extend([
        CharacterObject(149 + (center_data.CHARACTER_W + center_data.GAP) * i, 74, ch)
        for i, ch in enumerate("SUPPORT HIDE OBJECT")
        if ch != " "            
    ])

    stage.character_objects.extend([
        CharacterObject(113 + (center_data.CHARACTER_W + center_data.GAP) * i, 102, ch)
        for i, ch in enumerate("WHEN RUN OUT OF DISPLAY")
        if ch != " "
    ])
